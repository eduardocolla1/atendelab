...
async function editarTipo(id) {
    try {
        // ATENÇÃO: a action correta no TiposAtendimentosController é "buscar"
        const tipo = AtendeLabApi.toObject(await AtendeLabApi.get('tipos', 'buscar', { id }));
        novoTipo();
        tituloFormulario.textContent = 'Editar tipo';
        for (const [campo, valor] of Object.entries(tipo)) {
            const elemento = formTipo.elements.namedItem(campo);
